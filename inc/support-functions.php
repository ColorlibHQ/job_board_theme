...
function job_board_post_cats() {
	...
	if ( $cats ) {
		...
		$categories .= '<ul class="nav"><li><span><i class="fa fm fa-th-list"></i>' . esc_html( 'Catagory :', 'job_board' ) . '</span></li>';
...
// Blog categories
function job_board_featured_post_cat(){
...
// social media
if ( ! function_exists( 'job_board_social' ) ) {
	function job_board_social( $args = array() ) {
		...
		$url = job_board_opt( 'job_board_social_url' );
...
// header cart count
function job_board_cart_count( $class = '' ) {

	?>
	<div class="header-wrapicon2 <?php echo esc_attr( $class ); ?>">
		<img src="<?php echo esc_url( JOB_BOARD_DIR_ASSETS_URI ); ?>img/icons/icon-header-02.png" class="header-icon1 js-show-header-dropdown" alt="<?php esc_attr_e( 'ICON', 'job_board' ); ?>">
...
function job_board_contact7_form_content( $template, $prop ) {
...
add_filter( 'wpcf7_default_template', 'job_board_contact7_form_content', 10, 2 );