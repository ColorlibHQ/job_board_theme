...
function job_board_pagination( $args = array() ) {
    
    $defaults = array(
        'range'           => 4,
        'custom_query'    => FALSE,
        'previous_string' => esc_html__( '&laquo;', 'job_board' ),
        'next_string'     => esc_html__( '&raquo;', 'job_board' ),
        'before_output'   => '<div class="job_board-pagination-area col-12"><nav><ul class="pagination">',
        'after_output'    => '</ul></nav></div>'
    );
    
    $args = wp_parse_args( 
        $args, 
        apply_filters( 'job_board_pagination_defaults', $defaults )
    );
